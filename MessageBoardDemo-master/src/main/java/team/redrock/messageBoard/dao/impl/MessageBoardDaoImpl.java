...
public class MessageBoardDaoImpl implements MessageBoardDao {
    ...
    public int findByid(int id){
        String sql = "SELECT dian_count FROM message_board WHERE id = ?";
        Connection con = getConnection();
        PreparedStatement pstmt = null;
        ResultSet res = null;
    ...
    public void addCount(int id) {
        int dian_count = findByid(id);
        String sql = "update message_board set dian_count=?+1 where id=?";
        Connection connection = getConnection();
        PreparedStatement pstmt = null;
        try {
            pstmt = connection.prepareStatement(sql);
            pstmt.setInt(1,dian_count);
            pstmt.setInt(2,id);
            pstmt.executeUpdate();
        } catch (SQLException e) {
            try {
